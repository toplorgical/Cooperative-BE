...
class LoanServices {
  static async create(data: LoanProps, user: UserProps) {
    ...
    const messages = await LoanServices.constructMessage(
      result,
      user,
      loanType
    );
    ...
    const textMessage = await LoanServices.constructGuarantorTextMessage(
      result,
      user
    );
  ...
  private static async checkLoanEligibility(user: UserProps, data: LoanProps) {
    ...
    if (user.profileSetup !== "COMPLETED") {
      throw new ApplicationError(
        "Complete your profile to be eligible for loan"
      );
    }
    ...
    const pendingLoan = await LoanRepository.findOne({
      userId: user.id,
      status: "PENDING",
    } as LoanQueryProps);
    if (pendingLoan)
      throw new ApplicationError(
        "Request could not be completed as previous loans are still pending"
      );
    ...
    if (balance * 3 < Number(data.amount)) {
      throw new ApplicationError(
        "Insufficient savings balance. Topup your account to complete this process"
      );
    }
  }
  ...
  private static async checkGuarantorsEligibility(data: LoanProps) {
    ...
    for (const guarantor of guarantors) {
      const user = await UserRepository.findOne({
        registrationId: guarantor.registrationId,
      });
      if (!user)
        throw new NotFoundError(
          `Membership ID "${guarantor.registrationId}" is invalid`
        );

      if (user.profileSetup !== "COMPLETED") {
        throw new ApplicationError(
          `Guarantor with membership ID "${user.registrationId}" is ineligible`
        );
      }
  ...
  static calculateLoan(loan: LoanProps) {
    ...
    const monthlyRepayment =
      (principal * monthlyInterestRate) /
        (1 - Math.pow(1 + monthlyInterestRate, -loanDurationMonths)) || 0;
  ...
  static async cancel(id: number, userId: number) {
    ...
    if (loan.status !== "PENDING")
      throw new ApplicationError(
        "Request could not be completed as loan is not pending"
      );
  ...
  static async changeStatus(data: LoanProps) {
    ...
    if (loan.status !== "PENDING")
      throw new ApplicationError(
        "Request count not be completed as loan is not pending"
      );
  ...
  static async createLoanType(data: LoanTypeProps) {
    ...
    if (loanType)
      throw new ApplicationError(`Loan type (${data.name}) already exist`);
  ...
  static async updateLoanType(data: LoanTypeProps, id: number) {
    ...
    if (!loanType)
      throw new NotFoundError("The requested loan could not be found");
  ...
  static async updateGuarantorStatus(
    data: LoanGuarantorProps,
    id: number,
    userId: number
  ) {
    const guarantor = await LoanRepository.findOneGuarantor({ id, userId });
    // const guarantor = await LoanRepository.findOneGuarantor({ id });
    if (!guarantor) throw new NotFoundError("The Guarantor could not be found");
